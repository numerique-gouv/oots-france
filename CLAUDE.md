...
| Agent | What it does |
| --- | --- |
| [`tdd-nerd`](.claude/agents/tdd-nerd.md) | Renders what the TDD say of a subject, a ticket or the code, verbatim and linked; never an opinion. Read-only |
| [`contradicteur`](.claude/agents/contradicteur.md) | Reads one issue and reports what contradicts what, eight classes, each proven; proposes the correction, applies none. Read-only |
| [`spec-nerd`](.claude/agents/spec-nerd.md) | Writes, completes and statuses the Linear issues against the TDD, through `tdd-nerd` and `contradicteur`; asks the user only what no chapter settles, in one batch |
| [`ouvrier`](.claude/agents/ouvrier.md) | Delivers one ticket end to end in its own worktree — plan, implementation, PR, convergence — and never merges |

| Skill | What it does |
| --- | --- |
| [`plan-issue`](.claude/skills/plan-issue/SKILL.md) | Plans one issue against the text of the TDD, and gets the plan approved only if a question remains that no chapter settles |
...
| [`ci-en-fond`](.claude/skills/ci-en-fond/SKILL.md) | Watches a PR's CI in the background and reads its verdict; shared by `review-loop` and `ouvrier` |
| [`refondre-historique`](.claude/skills/refondre-historique/SKILL.md) | Rewrites a branch's history safely: backup tag, identical tree, `--force-with-lease` |
