...
**Four directories are versioned, plus one file: `.claude/skills/`, `.claude/agents/`, `.claude/statusline/`, `.claude/hooks/` and `.claude/settings.json`.** They describe *how work is done on this repository* — the review loop, the shipping sequence, the control of the backlog against the TDD, what a screen shows of an agent at work, and what the harness trims from a tool's answer before the model reads it — so they belong to the repository for the same reason this file does: a convention nobody can read is a convention nobody follows.

`.claude/statusline/` names each `ouvrier` by its ticket and its stage in the agents panel, in place of the `running` the harness would otherwise show for the three hours a ticket lasts. It reads its contract from `.claude/agents/ouvrier.md` — the seven verdicts, the stage declared in `.claude/etapes/` — so the two only stay in step by travelling together: adding or renaming a verdict means editing the script's own list in the same commit. `.claude/hooks/` holds the one `PostToolUse` hook: a Linear write (`save_issue`, `save_comment`) answers with the whole ticket, 10 Ko in median, and the hook keeps one line of it — `OK OOTS-189 Todo <url>` — so that an agent patching a ticket does not reread it at each call; a failed write never reaches the hook, so nothing is hidden. `.claude/settings.json` is what points the harness at them, through `${CLAUDE_PROJECT_DIR}` so that no machine's paths leak in; being project settings, it overrides whatever `~/.claude/settings.json` declares, here and nowhere else.

> [!IMPORTANT]
> `.claude/settings.json` makes this repository run a shell command on the machine of whoever opens it, under the same workspace-trust gate as a hook. Anything added there is read by every clone, so it stays limited to what a status line and that hook need.
...
| [`contradicteur`](.claude/agents/contradicteur.md) | Reads one issue and reports what contradicts what — the ticket against itself, against the state the repository is actually in, against the sources it invokes, against the sibling tickets that will write to the same file, against Linear's own state. Eight classes of incoherence, each proven in the pass; it proposes the correction and applies none. Read-only. Runs on Opus; `spec-nerd` launches it before raising a ticket to `Todo`, and a session can launch it directly |
| [`spec-nerd`](.claude/agents/spec-nerd.md) | Writes one issue from a light prompt, completes an existing one from new information, or opens the Linear project that carries a workstream. Confronts every question to the TDD through `tdd-nerd` before asking the user, then asks only the product, interface and genuinely undecidable questions, in one batch. Functional, never technical. Creates in `Backlog`, then sets the status itself: `Todo` when complete, `À compléter` when a wording or a decision is still missing — and every pass ends by sweeping the workstream it wrote into, raising to `Todo` the tickets nothing holds back any more. Runs on Fable, as a subagent: its questions come back in its report under `QUESTIONS`, for the session to relay |
...
| [`orchestrateur`](.claude/skills/orchestrateur/SKILL.md) | The user's single point of contact with the fleet. Picks the actionable issues out of the Linear backlog, runs several `ouvrier` agents on them in parallel, and sees each through to its PR; has `spec-nerd` write the ticket of a need stated in a sentence and relays its questions; at each delivery, has the user sort what the worker left out of scope, and turns into tickets only what a TDD chapter names or the user keeps |
| [`harness-engineer`](.claude/skills/harness-engineer/SKILL.md) | Rereads the harness against the latest runs — transcripts, reviews, handovers, PRs, tickets — and fixes it where the rule will actually be read: what was corrected by hand and should have been written, what is written and not followed, what is written twice or no longer true. Brings the local memories that describe how work is done here back into the versioned files. Writes an audit before touching anything, applies on a branch, opens the PR, never merges |

Everything else under `.claude/` is workshop material and stays local: `plans/`, `audits/`, `reviews/`, `reprises/`, `local_tasks/`, and `settings.local.json`. It is dated prose written for one moment of one task, and versioning it would age badly.

Local does not mean arbitrary — the skills read these directories, so their shape is a convention like any other. A plan goes to `.claude/plans/AAAA-MM-JJ-<sujet>.md` and is written **at the moment it is submitted for approval**, not after; a review to `.claude/reviews/`, an audit to `.claude/audits/`, same naming. One file per subject, keeping only the last revision: re-writing a plan overwrites its file, and a changed date renames it rather than leaving two versions side by side. `ship-plan` looks for the plan there, and finds nothing if it went elsewhere. A small follow-up that waits on a condition — a check to make in the first session after a merge, a memory to move once its PR is in — goes to `.claude/local_tasks/AAAA-MM-JJ-<sujet>.md`, which `orchestrateur` reads at the start of every run; its § Entrée owns the format.

> [!IMPORTANT]
> **A skill is read by whoever works here next, human or agent.** Keep them free of anything personal — an absolute path from one machine, a token, the name of a local VM. What a skill needs from the environment, it names as a variable or asks for. And it is written in plain words, a title saying what must be true and an abstract rule listing its cases — the rules, with the corrections that fixed them, are in [`harness-engineer/comment-on-ecrit.md`](.claude/skills/harness-engineer/comment-on-ecrit.md).
