...
> [!IMPORTANT]
> **Ces chemins sont déclarés depuis le 2026-09-17 : les changer coûte désormais un aller-retour avec FranceConnect+**, par le [second formulaire](https://demarche.numerique.gouv.fr/commencer/demande-modification-fs-fc). Ils ne dépendent pas du FranceConnect+ que l'on joue : le faux et le vrai ramènent l'usager sur les deux mêmes adresses de retour et lisent ses clés sur la même troisième.

Les deux retours ne se ressemblent pas. **Le retour de connexion redirige et n'affiche rien** : il échange le code, ouvre les jetons et envoie l'usager sur le formulaire de la démarche, ou le renvoie à son accueil avec la raison du refus — un code d'autorisation est à usage unique, et laisser dans l'historique une page qui en porte un invite à le rejouer. **Le retour de déconnexion, lui, est une page** : FranceConnect+ y ramène un usager qui n'a plus de session, et il n'y a nulle part où l'envoyer. Elle vérifie le `state` de la déconnexion et le dit quand il ne répond à aucune déconnexion demandée depuis ce navigateur.
...
**La cinématique européenne** ([lancement](https://docs.partenaires.franceconnect.gouv.fr/fs/passerelle-eidas/technique-eidas-authorize/)). L'appel à `authorize` porte `idp_hint=eidas-bridge`, qui mène l'usager droit au choix du pays, et demande le claim `amr` en `essential`. Au retour, `amr` **est un tableau de chaînes** — la page de la cinématique européenne le décrit comme une valeur unique, là où le cœur de FranceConnect+ en rend un tableau — qui contient `fc` pour une identité française et `eidas` pour celle d'un autre État membre. Ce dépôt lit les deux formes sans en privilégier une.
...
>
> En attendant, la démarche lit un claim nommé **`person_identifier`** — l'attribut `PersonIdentifier` du jeu minimal transposé dans la convention `snake_case` des claims du portail. Le nom est un choix de ce dépôt, il n'est documenté nulle part, et il fait partie de ce qui est **à confirmer** lors de la vérification manuelle sur le bac à sable. Rien ne le rend aujourd'hui : le chemin nominal est donc l'identité tenue **sans** identifiant, ce que le chapitre 2.1 §2.3.1.2 prévoit ; le lecteur existe pour que ce qui arriverait soit validé plutôt que porté tel quel dans une requête que `R-EDM-REQ-C040` (FATAL) refuserait.
...
**La démarche de démonstration parle à FranceConnect+, et elle seule.** L'accueil offre une carte par FranceConnect+ que le déploiement déclare — le faux du dépôt, le vrai, ou les deux —, et le bouton de chaque carte ouvre la cinématique européenne du sien : le retour de connexion échange le code, ouvre l'ID Token et la réponse UserInfo, et l'identité obtenue s'affiche — non modifiable — sur la page des justificatifs ; la déconnexion de l'exploitant ferme aussi la session du portail qui avait attesté. Le [chantier Linear](https://linear.app/pole-api/project/oots-france-le-portail-de-demonstration-484068da336c) porte la suite, et [espace_administration.md](espace_administration.md#la-démarche-de-démonstration) décrit les pages.

**Un FranceConnect+ se déclare par trois variables**, et c'est tout ce qui sépare le faux, le bac à sable et la production. Le déploiement en porte deux jeux, nommés pour dire lequel des deux ils déclarent : `URL_FAUX_FRANCE_CONNECT`, `IDENTIFIANT_CLIENT_FAUX_FRANCE_CONNECT` et `SECRET_CLIENT_FAUX_FRANCE_CONNECT` d'un côté, `URL_VRAI_FRANCE_CONNECT`, `IDENTIFIANT_CLIENT_VRAI_FRANCE_CONNECT` et `SECRET_CLIENT_VRAI_FRANCE_CONNECT` de l'autre. La première de chaque jeu est l'*issuer* dont le document de découverte est déduit. Un jeu renseigné à moitié, ou aucun des deux renseigné, refuse le démarrage en nommant les variables manquantes. Aucune adresse d'endpoint n'est écrite dans le code : toutes se lisent dans la découverte du FranceConnect+ que l'on joue, et les deux adresses de retour se composent de `URL_OOTS_FRANCE` et des chemins de `config/routes.rb` — jamais de la requête, FranceConnect+ refusant une `redirect_uri` qu'il n'a pas déclarée.

Ce que la démarche demande et vérifie, règle par règle, est dans [OOTS-179](https://linear.app/pole-api/issue/OOTS-179) ; ce que l'intégration continue en joue est dans [test_e2e.md](test_e2e.md).

**Le requêteur, lui, ne parle pas à FranceConnect+.** L'identité y entre par le jeton du bénéficiaire qu'un fournisseur de service français chiffre pour lui — c'est [oots_context.md](oots_context.md#côté-evidence-requester-la-france-demande-un-justificatif) qui le décrit —, et le niveau de garantie qui part dans la requête est celui que ce jeton transporte : le requêteur écrit ce qu'on lui a transmis, sans valeur par défaut, faute de quoi il annoncerait un niveau que personne n'a attesté. Obtenir ce niveau de FranceConnect+, par le claim `acr`, appartient au portail — et c'est exactement ce que la démarche de démonstration fait désormais.

Le scénario retenu pour la démonstration est un étudiant danois, identifié par sa propre identité danoise à travers la passerelle eIDAS de FranceConnect+, qui demande à la France un justificatif pour une démarche **française** de bourse d'études — l'usager est étranger, mais le requêteur et le fournisseur sont tous deux la France, pour rester sur une boucle que l'on maîtrise de bout en bout. Ce que cela exige de FranceConnect+, dans l'ordre : un déploiement joignable, l'habilitation, l'accès au bac à sable, puis l'activation de la passerelle. **La démarche est déclarée sur le bac à sable** depuis le 2026-09-17, date à laquelle le déploiement `https://oots.infra.data.gouv.fr` a appelé son `/token` avec ses identifiants. Les adresses qu'elle déclarera, elles, existent — [ci-dessus](#les-adresses-que-ce-dépôt-déclarera).

> [!IMPORTANT]
> **La vérification manuelle sur le bac à sable reste due**, et elle seule dira ce que FranceConnect+ rend vraiment à un usager européen : les claims, `acr`, `amr`, et si un claim porte l'identifiant eIDAS. Ce qui tourne aujourd'hui est vérifié contre le faux d'[OOTS-186](https://linear.app/pole-api/issue/OOTS-186), qui reproduit la documentation et les sources, pas le bac à sable. Le résultat de cette vérification s'écrit ici.
