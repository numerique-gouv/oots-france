...
**La démarche de démonstration parle à FranceConnect+, et elle seule.** Le bouton de son accueil ouvre la cinématique européenne, le retour de connexion échange le code, ouvre l'ID Token et la réponse UserInfo, et l'identité obtenue s'affiche — non modifiable — sur la page de confirmation ; la déconnexion de l'exploitant ferme aussi la session du portail. Le [chantier Linear](https://linear.app/pole-api/project/oots-france-le-portail-de-demonstration-484068da336c) porte la suite, et [espace_administration.md](espace_administration.md#la-démarche-de-démonstration) décrit les pages.
