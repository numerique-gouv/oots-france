...
**La démarche de démonstration parle à FranceConnect+, et elle seule.** Le bouton de son accueil ouvre la cinématique européenne, le retour de connexion échange le code, ouvre l'ID Token et la réponse UserInfo, et l'identité obtenue s'affiche — non modifiable — sur la page des justificatifs ; la déconnexion de l'exploitant ferme aussi la session du portail. Le [chantier Linear](https://linear.app/pole-api/project/oots-france-le-portail-de-demonstration-484068da336c) porte la suite, et [espace_administration.md](espace_administration.md#la-démarche-de-démonstration) décrit les pages.

La configuration tient en trois variables, et c'est tout ce qui sépare le faux, le bac à sable et la production : `URL_FRANCE_CONNECT`, l'*issuer* dont le document de découverte est déduit, `IDENTIFIANT_CLIENT_FRANCE_CONNECT` et `SECRET_CLIENT_FRANCE_CONNECT`. Aucune adresse d'endpoint n'est écrite dans le code : toutes se lisent dans la découverte, et les deux adresses de retour se composent de `URL_OOTS_FRANCE` et des chemins de `config/routes.rb` — jamais de la requête, FranceConnect+ refusant une `redirect_uri` qu'il n'a pas déclarée.
