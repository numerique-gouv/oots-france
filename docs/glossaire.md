...
| **NoMatch** — *match type* | La valeur que `sdg:MatchType` prend quand un État membre déclare **savoir** qu'aucun de ses justificatifs ne satisfait une exigence dans cette juridiction : la combinaison est alors vide à dessein, dans une réponse en succès. À distinguer d'`EB:ERR:0001`, qui dit que l'Evidence Broker n'a aucune information ([chapitre 3.2.4](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932939), § 4.3.5). Le pays peut y joindre une `sdg:MatchDescription` expliquant pourquoi. | `EvidenceTypeList::NO_MATCH`, `EvidenceTypeList#no_match?` |
| **Service de données** — *data service* | Ce que le Data Service Directory publie pour un type de justificatif dans un pays : l'identifiant qu'il attribue à ce couple, le format, la langue, le modèle de données, le niveau de garantie, et les fournisseurs qui le délivrent, chacun avec son point d'accès. | `DataService` |
| **Modèle de données** — `ConformsTo` | L'URL pérenne, au Semantic Repository, du schéma auquel un justificatif structuré se conforme. Le DSD la publie pour une distribution XML ou JSON, et la requête la reprend pour dire au fournisseur contre quel modèle produire le document ; sur un format non structuré, l'écrire est une erreur fatale. | `DataService#distribution_conforms_to` |
| **Document associé** — `AssociatedDocumentRequest` | Ce qu'un requêteur demande de recevoir **en plus** du justificatif principal, parmi trois valeurs et trois seulement : une annexe, une version lisible par un humain, une traduction ([chapitre 4.5.1](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932961)). C'est une demande du requêteur, jamais une donnée d'annuaire. | `AssociatedDocument` |
...
| **Niveau de garantie** — `LevelOfAssurance` | La force du moyen d'identification qui a authentifié la personne : `Low`, `Substantial` ou `High`. Obligatoire dans la requête dès qu'une personne y figure. À ne pas confondre avec l'`AuthenticationLevelOfAssurance` que publie le DSD, qui est le niveau qu'un service de données **exige**. | `NaturalPerson::LEVEL_OF_ASSURANCE` et `LegalPerson::LEVEL_OF_ASSURANCE` (bouchons), `DataService#level_of_assurance` |
| **Personne morale** — `LegalPerson` | L'organisation pour le compte de laquelle un justificatif est demandé, quand le sujet n'est pas une personne physique ; elle porte sa raison sociale, son identifiant eIDAS, son niveau de garantie, et autant d'identifiants sectoriels facultatifs qu'elle en a, chacun sous son propre schéma. | `LegalPerson` |
