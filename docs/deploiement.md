...
| `.env.oots` | `UTILISATEUR_BASE_DE_DONNEES`, `MOT_DE_PASSE_BASE_DE_DONNEES`, `NOM_BASE_DE_DONNEES` alignés sur `.env.postgres` ; `MOT_DE_PASSE_APPLICATIF_BASE_DE_DONNEES` | le rôle applicatif est créé par `make setup` avec ce mot de passe |
