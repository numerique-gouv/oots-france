...
L'[espace d'administration](espace_administration.md) le donne à lire, sous `/admin/journal` : la liste des événements, la fiche de l'un d'eux, et la recherche par personne. Le journal d'un échange donné se lit, lui, sur la fiche de cet échange, sous `/admin/journal/exchanges` — que les deux sens alimentent, si bien qu'une seule liste porte tout ce qui circule.
...
- **Deux champs de corrélation manquent**, tous deux écrivables sans attendre : l'**identifiant de contenu MIME** des pièces référencées par `rim:RepositoryItemRef`, dont `evidence_fingerprint` n'écrit que le type, et la **`PreviewLocation`**, que le chapitre réclame dans les deux flux et qu'`Exchange` seul retient — `AuditTrail#received_error` n'en consigne rien.
