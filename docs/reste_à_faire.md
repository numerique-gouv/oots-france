...
> [!WARNING]
> **Une réponse différée fait échouer l'échange, puis condamne la vraie réponse.** `EvidenceResponseParser` ne lit pas l'attribut `status` : une réponse `Unavailable`, par laquelle un fournisseur annonce la date à laquelle le document sera disponible (`R-EDM-RESP-C005`), ne porte aucun justificatif, donc `IncomingMessage::Process` la juge illisible et passe la conversation en `failed` — un état réglé, où le justificatif envoyé plus tard est refusé en `already_settled`. Lire `status` et `ResponseAvailableDateTime` ne dépend d'aucun autre chantier.
...
L'[espace d'administration](espace_administration.md) le donne à lire, sous `/admin/journal` : la liste des événements, la fiche de l'un d'eux, et la recherche par personne. Le journal d'un échange donné se lit, lui, sur la fiche de cet échange, sous `/admin/journal/conversations` — que les deux sens alimentent, si bien qu'une seule liste porte tout ce qui circule.
...
- **Le corps RegRep lui-même n'est conservé nulle part.** Les deux tableaux du chapitre demandent au requêteur le type MIME et le **contenu entier de la première partie MIME** — la requête, la réponse ou l'`rs:Exception` telles qu'elles ont circulé. `audit_events` n'en garde que des identifiants, et `retention_downloaded="0"` fait effacer la charge par Domibus dès le `retrieveMessage` : le partage « protocole à la passerelle, métier ici » laisse cette ligne à personne.
- **Deux champs de corrélation manquent**, tous deux écrivables sans attendre : l'**identifiant de contenu MIME** des pièces référencées par `rim:RepositoryItemRef`, dont `evidence_fingerprint` n'écrit que le type, et la **`PreviewLocation`**, que le chapitre réclame dans les deux flux et que `Conversation` seule retient — `AuditTrail#received_error` n'en consigne rien.
...
| [4.8 — Journalisation des échanges](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932926) | Partiel | La couche métier est écrite et conservée douze mois ; le corps RegRep n'est gardé nulle part et la chaîne de non-répudiation ne se rejoue pas automatiquement |
