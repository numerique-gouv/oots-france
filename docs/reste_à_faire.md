...
Un État membre ne fait pas que **lire** ces annuaires : il doit aussi les **alimenter**. Le [chapitre 3.6.3](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932943) définit pour cela une seconde interface, dite de *life cycle management*, qui n'a rien de commun avec les appels REST : c'est un message RegRep transporté par eDelivery, sur un service `LifecycleManager` distinct de celui des échanges, émis par une autorité nationale unique habilitée. Ce que la France doit y déclarer est décrit au [chapitre 3.1.6](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932940) : un couple type de justificatif × distribution, l'organisation qui le détient, la passerelle qui la sert, et les liens entre les trois.
...
**Ce qui reste.** Ce que ce raccordement rend possible sans le traiter, et ce que les réponses des annuaires portent sans qu'on le lise :
...
- **Le dialogue de désambiguïsation.** Quand un pays a plusieurs fournisseurs pour un même justificatif, le DSD répond une erreur `DSD:ERR:0005` qui demande une précision à poser à l'usager — « dans quelle ville êtes-vous né ? ». Le dépôt ne la distingue pas d'une panne : `Directories::CommonServices` ne traduit que `DSD:ERR:0001` et `0002`, si bien que celle-ci ressort en `502`, comme si l'annuaire était en dérangement. Elle mérite au contraire d'être rendue à l'appelant sous une forme qui lui permette de poser la question — l'écran appartient au portail de démarche, jamais à ce composant —, puis recopiée dans la requête suivante. Le [chapitre 3.1.3](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932962) en donne les trois formes : une liste de codes, une question fermée, une saisie libre.
- **La différence entre « il n'y a rien » et « on n'en sait rien ».** Le [chapitre 3.2.4](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932939) fait déclarer à un État membre une liste vide marquée `NoMatch` quand il sait n'avoir aucun justificatif à opposer à une exigence, et réserve `EB:ERR:0001` au cas où il n'a rien déclaré du tout. Les deux ne se traitent pas pareil : le premier ferme la question, le second dit qu'elle reste ouverte. `EvidenceTypesResponseParser` ne lit ni `MatchType` ni le `MatchDescription` qui l'explique en clair, et le dépôt confond donc les deux en une absence de type.
- **Le ressort des juridictions.** Une exigence comme une liste de types peuvent ne valoir que pour une région, une communauté linguistique ou une taille d'entreprise, ce que porte `AdditionalJurisdictionClassification`. Le dépôt ne lit que le code pays, et retiendrait donc pour toute la Belgique ce qui ne vaut que pour sa communauté néerlandophone.
- **La signature des réponses, dans son algorithme préféré.** Le [chapitre 3.6.2](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932954) recommande `EdDSA` et n'autorise `ES256` qu'en second choix ; `CommonServicesSignature` n'implémente que le second. Une instance qui basculerait sur le premier serait refusée — c'est le bon repli, mais il laisse un travail à faire.
...
L'inscription se fait par deux voies, et elles ne coûtent pas la même chose. L'**interface graphique** de la Commission ([CS Admin Tool](https://cs.oots.tech.ec.europa.eu/home)), que l'article 5(2) du [règlement d'exécution](https://eur-lex.europa.eu/eli/reg_impl/2022/1463/oj) lui impose de fournir, ne demande aucun code : c'est la saisie manuelle qui lève le blocage. L'**interface LCM**, elle, est un chantier entier — message, gabarit, PMode et habilitation, dont le dépôt n'a rien —, et ne se justifie que le jour où le jeu de données français change assez souvent pour que la saisie devienne pénible. S'y ajouterait alors la publication d'une **classification propre à la France** au format Genericode ([3.5.2](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932945)), si la désambiguïsation ci-dessus demande une question à l'usager.

> [!WARNING]
> **Une soumission LCM remplace l'intégralité des données du pays.** Le [chapitre 3.6.3](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932943) est explicite : « *any existing data previously submitted to the Common Service for that Member State is replaced* ». Il n'existe ni soumission incrémentale, ni protocole de suppression. Toute automatisation devra donc reconstruire le jeu complet à chaque envoi, sous peine d'effacer ce qu'elle n'a pas remis.
...
**Une décision à prendre en même temps : eIDAS 1 ou eIDAS 2.** La 2.0 fait cohabiter deux jeux d'attributs dans les mêmes slots — celui du règlement d'exécution 2015/1501, et celui du portefeuille européen d'identité défini par le [règlement d'exécution 2024/2977](https://eur-lex.europa.eu/eli/reg_impl/2024/2977/oj). Ils ne se distinguent que par le `schemeID` de l'identifiant, `eidas` ou `eidas2`, mais ce choix en entraîne d'autres : sous `eidas2`, le niveau de garantie vaut obligatoirement `High`, le lieu de naissance et la nationalité deviennent obligatoires, le genre s'exprime en codes numériques et le nom de naissance en deux valeurs séparées par un `#`. Le [chapitre 4.5.1](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932961) avertit par ailleurs que l'usage du portefeuille en production « *is subject to confirmation of provision of a suitable legal basis* » : viser `eidas2` aujourd'hui, c'est développer contre une base juridique qui n'est pas encore acquise.
...
**Ce qu'il faut construire.** Côté demandeur : les deux slots, la fabrication d'une adresse de retour à usage unique et à durée limitée, le point d'entrée qui accueille le retour, et la reprise de la démarche là où l'usager l'avait laissée — en le ré-authentifiant si sa session a expiré pendant qu'il était à l'étranger. Le dépôt doit aussi savoir présenter le départ : le `PreviewDescription` que porte l'erreur donne, en plusieurs langues, le texte à afficher avec le lien. Côté fournisseur : un espace de prévisualisation complet — c'est la seule interface humaine du dépôt, et le seul endroit où les règles d'accessibilité s'appliquent.

Le [chapitre 4.9](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932935) contraint plus l'**adresse** que l'écran, et ces contraintes-là ne s'improvisent pas : `https://` obligatoire, méthode `GET`, aucun fragment, une adresse imprévisible — un UUID v4 y suffit — et propre à une seule requête, donc à un seul usager, un seul type de justificatif et un seul couple demandeur/fournisseur. S'y ajoute un cycle de vie complet : inaccessible avant que la première requête ne soit arrivée, ouverte pendant un temps borné, réouvrable tant que l'usager n'a pas terminé, close définitivement dès qu'il a décidé. L'espace doit enfin mémoriser tout ce que portait la première requête — tous les slots sauf l'horodatage, et les propriétés ebMS — pour le relire quand l'usager se présente.

> [!NOTE]
> **Un usager qui refuse tout n'est pas un échec.** La réponse au second échange porte alors une liste d'objets **vide**, et c'est un aboutissement normal du flux. Le dépôt ne sait pas encore la lire : voir le chantier 5.
...
Viennent avec le premier fournisseur réel trois obligations que le PDF d'exemple n'a jamais fait sentir. Les **métadonnées** cessent d'être des constantes : la date d'émission, la période de validité, et l'**autorité émettrice**, que le [chapitre 4.5.2](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932951) distingue expressément du fournisseur — une plateforme qui agrège des mairies n'a pas émis les actes qu'elle transmet. Les **plafonds** du [chapitre 4.7](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932931) deviennent atteignables — 25 Mo par justificatif, 100 Mo pour l'ensemble, avant compression — et rien ne les vérifie aujourd'hui. Et l'**octet exact remis** doit être conservé, pour la raison expliquée au chantier 7.
...
> [!WARNING]
> **Trois réponses parfaitement conformes sont aujourd'hui tenues pour illisibles.** `RetrievedMessageParser#evidence` va chercher une pièce jointe `application/pdf` et abandonne l'échange si elle n'y est pas. Or la spécification prévoit trois cas où elle n'y est légitimement pas : la **liste vide** que le [chapitre 4.9](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932935) impose quand l'usager refuse le justificatif ou qu'il n'y en a aucun ; la réponse **différée** du [chapitre 4.5.2](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932951), dont le statut `Unavailable` s'accompagne d'une date de disponibilité et d'une liste vide ; et tout **justificatif structuré**, qui arrive dans un autre format. Dans les trois cas, l'échange est clos en « message illisible » au lieu d'être rendu au demandeur pour ce qu'il est : un refus de l'usager, un rendez-vous ultérieur, un document exploitable. C'est un défaut à corriger, et il ne dépend d'aucun autre chantier.

Une dernière imprécision, de sens inverse : le dépôt est plus **tolérant** que la règle. `ErrorResponseParser` et la validation de `Conversation#preview_location` acceptent une adresse de prévisualisation en `http://`, là où `R-EDM-ERR-C019` et `R-EDM-REQ-C005` imposent `https://`. Le resserrement tient en une ligne, et se fera naturellement avec le [chantier 3](#3-la-prévisualisation), qui fait de cette adresse une entrée de la seconde requête.
...
**Ce que fait le dépôt.** Rien : le seul format traité de bout en bout est le PDF, écrit en dur aussi bien dans la lecture des messages reçus que dans le type de pièce jointe attendu. C'est le **bouchon 8**. Le lien vers le modèle de données est perdu dès la lecture des annuaires : `EvidenceTypesResponseParser` retient le `Format` de la distribution structurée que publie l'Evidence Broker, et jette le `ConformsTo` qui l'accompagne — c'est-à-dire précisément l'URL du modèle.
...
> [!IMPORTANT]
> **Publier du XML sans modèle convenu n'est pas une option.** Le [chapitre 3.1.4](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932957) impose que tout type de justificatif structuré qui ne se rattache à aucun modèle OOTS soit accompagné d'une version lisible par un humain, déclarée au Data Service Directory. Le jour où la France publiera un justificatif structuré, elle devra donc soit passer par un modèle convenu au niveau européen, soit publier un PDF à côté.
...
- **Le contenu exact du justificatif n'est pas conservé.** Le chapitre est pourtant explicite : « *the evidence provider must have a record, for each piece of evidence it exchanged […] to the actual identical evidence content* ». Le journal en garde l'empreinte, ce qui permet de **vérifier** qu'un document produit par un tiers est bien celui qui a circulé, mais pas de le **produire** si le litige porte là-dessus. Trancher demande de peser cette obligation contre la minimisation des données : conserver l'octet, ou établir que le fournisseur amont le fait et savoir le lui redemander.
...
- **La découverte dynamique par SMP** est facultative en 2.0 et annoncée comme obligatoire par la suite — le [chapitre 4.7](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932931) écrit « *expected to become mandatory in a future version* ». Elle évite de notifier tous les correspondants à chaque renouvellement de certificat. Ce qu'elle suppose est déjà écrit, et vaut d'être lu avant de s'engager : eDelivery AS4 **1.16** au minimum, avec l'extension *Dynamic Sender* dans sa variante « découverte fondée sur un C3 connu » — c'est le DSD qui donne ce C3 —, un client [SMP 2.1](https://ec.europa.eu/digital-building-blocks/sites/spaces/DIGITAL/pages/467117984/SMP+specifications), et une authentification OAuth par mTLS auprès d'un serveur d'autorisation central, sous la portée `oots-query`. À vérifier au passage : que la version de Domibus déployée sache le faire, ce dont [versions_domibus.md](versions_domibus.md) est le propriétaire. À prévoir, pas à faire maintenant.
- **Les certificats du réseau réel.** Les magasins de `domibus/` sont auto-signés et fabriqués sur la machine du développeur ; le [chapitre 4.7](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932931) attend des certificats conformes à ses profils, de préférence issus de la PKI eDelivery, et impose en AS4 1.16 un **certificat RSA unique pour la signature et le chiffrement**, de plus de 3000 bits — une contrainte dictée par les limites de certains produits AS4, et qu'il vaut mieux découvrir maintenant que le jour du raccordement. Le profil TLS du [chapitre 3.7](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932927) — TLS 1.2 au minimum, suites imposées, DNSSEC recommandé — relève du même moment.
...
| [3.1 — DSD](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932957) | Partiel | Le dialogue de désambiguïsation `DSD:ERR:0005` ; **la France n'y est pas inscrite** |
| [3.2 — Evidence Broker](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932939) | Partiel | Interrogé correctement ; `MatchType`, `AdditionalJurisdictionClassification` et le lien vers le modèle de données ne sont pas lus |
...
| [3.5 — Listes de codes](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932952) | Partiel | Seuls les codes d'erreur sont transcrits ; les autres listes ne sont ni chargées ni vérifiées, et aucune classification française n'est publiée |
| [3.6 — API commune](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932954) | Partiel | Les requêtes sont conformes ; la signature n'est vérifiée qu'en `ES256`, et l'interface d'alimentation ([3.6.3](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932943)) n'existe pas |
...
| [4.5.2 — Réponse](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932951) | Partiel | À l'écriture : documents complémentaires, réponse différée, langue et conformité. À la lecture : liste vide et réponse différée tenues pour illisibles |
...
| [4.7 — eDelivery](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932931) | Partiel | Configuration statique conforme ; SMP à prévoir, plafonds de taille non vérifiés, certificats de démonstration |
...
| --- | --- | --- |
| 1. Common Services — interroger | *fait* | 5 (écriture), 6 |
| 1. Common Services — s'y inscrire | Une saisie hors dépôt : l'outil d'administration de la Commission | Tout échange réel où la France est sollicitée |
...
- **Les deux réponses conformes tenues pour illisibles** (chantier 5). La liste vide et la réponse différée ne demandent qu'à être lues pour ce qu'elles sont ; rien n'en dépend, et tout ce qui suit en dépend.

### Ce qui ne dépend pas de ce dépôt

Trois chantiers n'avancent pas par le seul code, et leur délai est celui d'accords à obtenir ou d'une démarche administrative :
...
- **L'inscription au Data Service Directory** (chantier 1) est une saisie dans l'outil d'administration de la Commission. C'est la seule de ces trois qui ne demande ni développement ni négociation technique, et la seule dont l'absence rend la France invisible.
