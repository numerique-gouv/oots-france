...
- **`R-EDM-REQ-C004` et `C069` n'existent qu'en 1.2.5** : la langue du `rim:LocalizedString` que le slot `Procedure` porte sur cette ligne, et l'`xml:lang` de la `query:QueryRequest` elle-même. La 2.0 ne publie ni l'une ni l'autre — la langue a déménagé dans chaque `sdg:DistributedAs` (rupture 3).
