...
| Le suivi de la démarche reste indéfiniment « en cours », sans rien dans les journaux de l'application | la passerelle a été configurée sur un autre port que celui que `web` écoute : elle pousse ses notifications sur `web:<PORT_OOTS_FRANCE>`, et rien n'écoute en face. `scripts/ci/diagnose_domibus.sh` signale l'écart ; le remède est dans [domibus_context.md](domibus_context.md#comment-oots-france-utilise-domibus) |
