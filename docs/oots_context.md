...
1. Un fournisseur de service appelle `GET /requete/pieceJustificative?codeDemarche=…&codePays=…&idRequeteur=…&beneficiaire=…` (`EvidenceRequestsController`), en y ajoutant `idConversation` s'il mène le même usager à travers plusieurs demandes, et `idExigence` s'il veut nommer laquelle des exigences de la démarche il demande — l'URL du dépôt sémantique entière, telle que l'Evidence Broker la publie. Sans lui, la requête porte la première exigence pour laquelle l'Evidence Broker publie un type dans le pays ; avec lui, elle porte cette exigence-là et refuse en `422` plutôt que de se replier sur une autre. Une requête porte une exigence : le [chapitre 4.5.1 §3.1](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932961) n'en laisse voyager plusieurs ensemble que si un seul type de justificatif les prouve toutes. Le paramètre `beneficiaire` est un [**JWE**](https://datatracker.ietf.org/doc/html/rfc7516) (*JSON Web Encryption*) chiffré avec la clé publique exposée par `GET /auth/cles_publiques`. Il est ouvert par `BeneficiaryToken`, avec la clé privée que fournit `CLE_PRIVEE_JWK_EN_BASE64`, puis vérifié contre le JWKS que le fournisseur de service publie à sa **propre** URL `/auth/cles_publiques`. Deux jeux de clés distincts interviennent donc : celui d'OOTS-France pour ouvrir l'enveloppe, celui du fournisseur pour authentifier son contenu. Cette signature n'atteste que l'émetteur, jamais sa légitimité à agir pour le bénéficiaire déclaré.

   Ce que le jeton porte, une fois ouvert : `niveauGarantie`, `nomUsage`, `prenom` et `dateNaissance`, tous quatre obligatoires — un jeton auquel il en manque un est refusé en `422` —, puis `identifiantEidas`, `lieuNaissance` et `sexe`, que le fournisseur de service transmet quand il les a reçus. `niveauGarantie` est l'un des trois codes des TDD (`Low`, `Substantial`, `High`) et se transmet tel quel : c'est la responsabilité du requêteur, au [chapitre 4.5.1 §3.6](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932961), que le niveau écrit dans la requête soit celui de l'authentification réellement effectuée, ce qui interdit toute valeur par défaut. `identifiantEidas` a la forme `XX/YY/Z…Z` que [`R-EDM-REQ-C040`](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932928) impose. `sexe` s'écrit dans le vocabulaire de FranceConnect+ — `male`, `female`, `unspecified` —, que `BeneficiaryToken` traduit ([glossaire](glossaire.md)) ; `lieuNaissance` est un texte libre.
...
3. Il **n'attend pas** la réponse : elle revient sur une autre connexion, par un répartiteur qui tourne à son propre rythme. Un `Exchange` est ouvert et l'appelant reçoit aussitôt un `202` portant son identifiant — la corrélation que décrit le [chapitre 4.10](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932922). Il reçoit aussi l'identifiant de conversation, celui de l'usager et de sa session, qu'il peut avoir fourni lui-même par `idConversation` pour rattacher deux demandes successives. Quand la réponse arrive, le justificatif est transmis au fournisseur de service sur sa propre URL — `POST <url>/oots/document?echange=…&conversation=…`, le corps étant le PDF nu, sous son propre type MIME. Les deux identifiants voyagent parce que le [chapitre 4.4 §4.3.2](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932919) donne à chacun son office — l'`ExchangeId`, qu'OOTS introduit « *to correlate all messages belonging to a single Evidence Request-Response exchange* », et le `ConversationId`, dont les messages « *MUST relate to that user* » —, et parce que le [chapitre 1 §7.9](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932933) veut qu'un maillon national « *include all data elements required for enabling logging for end-to-end correlation* » ; un fournisseur menant deux usagers de front ne saurait sinon à qui remettre quoi. Ils portent les noms mêmes que le `202` emploie. Un fournisseur qui ne reconnaît pas l'échange refuse la remise, et OOTS-France en prend acte : le [chapitre 4.10 §4.1](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932922), informatif, veut l'événement consigné pour investigation — le refuser est une décision de ce dépôt, que ce paragraphe ne demande pas. L'état de l'échange, lui, se relit à `GET /requete/:exchange_id`.

La [démarche de démonstration](espace_administration.md) est **un client de ce contrat, et non un chemin de faveur** : elle appelle `GET /requete/pieceJustificative` en HTTP, comme le ferait le serveur d'un fournisseur de service externe. Elle est donc inscrite dans `DONNEES_REQUETEURS` sous son propre SIRET (`IDENTIFIANT_REQUETEUR_DEMARCHE`, distinct d'`IDENTIFIANT_FOURNISSEUR_FRANCAIS` : elle joue C1 quand celui-ci joue C4), sous le nom « Université de démonstration » et à l'adresse que `DONNEES_REQUETEURS` lui donne — sur la pile locale `http://web:<PORT_OOTS_FRANCE>/demo`, un nom de service et non `localhost`, la remise étant faite par le worker, qui tourne dans un autre conteneur — d'où OOTS-France lit ses clés de signature à `/demo/auth/cles_publiques`, publiées à partir de `CLE_PRIVEE_JWK_SIGNATURE_DEMARCHE_EN_BASE64`. Elle signe son jeton du bénéficiaire en ES256 et le chiffre pour la clé qu'elle lit à `/auth/cles_publiques`, jamais pour une clé qu'elle dériverait elle-même. L'usager y demande explicitement l'usage d'OOTS ([chapitre 1 §3.3](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932933)), puis confirme sur une page qui lui nomme le fournisseur et le type de justificatif que la résolution rend — l'exigence 27 du même chapitre — et c'est ce geste qui fait partir la requête, le [chapitre 4.5.1 §2.7](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932961) liant l'`IssueDateTime` à son instant. Elle tient enfin les deux bouts du contrat comme un vrai fournisseur : elle reçoit le justificatif sur `POST /demo/oots/document` et relit l'état de son échange à `GET /requete/:exchange_id`, sans jamais lire la table `exchanges` qu'elle côtoie.
...
- démarche `00`, la **vérification système** d'OOTS : une `ExecuteQueryResponse` complète, qui reprend les données de la requête (bénéficiaire, requêteur, type de justificatif) et porte un vrai PDF en pièce jointe — un justificatif de démonstration engendré au moment de la réponse, qui dit le type demandé, l'usager et l'instant de sa production ;
- démarche `T1`, le **financement des études**, celle de la démonstration de l'Université : la même réponse, avec un justificatif engendré de la même façon — la France n'en détient aucun de réel ;
