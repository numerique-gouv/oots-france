...
Un annuaire qui **refuse** est une information, pas une panne : la page rend son code — `EB:ERR:0001`, `DSD:ERR:0001` — et répond `200`. Le titre de l'alerte, lui, dit ce qui s'est passé — « L'annuaire a refusé » — et non le code, que le corps ouvre déjà : le remettre au-dessus le dirait deux fois. Un annuaire **injoignable** répond `502`. Sur la chaîne de requêtes, un refus à la dernière étape laisse affichées les réponses des précédentes — le cas d'un pays qu'aucun service de données ne couvre pour le type demandé.
