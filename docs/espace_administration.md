...
| `/admin/journal` | Le [journal des échanges](journal_des_echanges.md), du plus récent au plus ancien, filtrable par type d'événement, démarche, pays, requêteur et période. C'est la seule vue qui porte **le refus prononcé avant qu'aucun échange soit ouvert**, qu'aucun échange ne peut représenter. |
...
| `/admin/journal/exchanges` | La liste des échanges, du plus récent au plus ancien, filtrable par état, pays, démarche, requêteur, **conversation** et période. L'état est rendu en pastille DSFR. **Les deux sens y figurent** : ce que la France demande, et ce qu'on lui demande. |
| `/admin/journal/exchanges/:id` | Le détail d'un échange, **`error_description` comprise** — la raison d'un échec, qu'aucune autre interface n'expose (voir le [chantier 10](reste_à_faire.md#10-ce-que-lappelant-apprend-dun-échec)) — et, sous lui, **le journal de cet échange**. La ligne « Conversation » est un lien vers la liste filtrée sur elle : les autres échanges de la même session d'usager. |
...
**L'état d'un échange n'est pas le journal de l'article 17**, et les deux se lisent séparément : `/admin/journal` montre ce qu'un échange a laissé comme trace, `/admin/journal/exchanges` où il en est. Les échanges tiennent sous le journal parce qu'on les y cherche — une ligne du journal mène à l'échange qu'elle concerne —, non parce que l'un serait un extrait de l'autre. Le journal a sa propre page de documentation, [journal_des_echanges.md](journal_des_echanges.md).
