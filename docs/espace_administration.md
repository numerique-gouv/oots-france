...
| `/admin/journal` | **La seule liste de la console**, intitulée « Journal des évènements » parce que c'est ce qu'elle liste : le [journal de l'article 17](journal_des_echanges.md), du plus récent au plus ancien, filtrable par type d'événement, **échange**, **conversation**, démarche, pays, requêteur et période. C'est là qu'on cherche, et de là qu'on descend. Elle porte aussi **le refus prononcé avant qu'aucun échange soit ouvert**, qu'aucune autre page ne peut représenter. |
