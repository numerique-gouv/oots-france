...
- **« *For evidence content referenced using `rim:RepositoryItemRef` elements, MIME type and MIME content identifier* »** — étoilée pour le requêteur, les points d'accès **et** le fournisseur dans le tableau du flux réponse. Ce sont `evidence_mime_type` et `evidence_content_id` : le `cid:` que l'`eb:PartInfo` déclare et que le `rim:RepositoryItemRef` du corps désigne, la même chaîne des deux côtés. C'est ce qui relie un justificatif à la partie MIME qui l'a porté, donc à l'empreinte que la passerelle a signée de cette partie-là — le maillon que le `message_id` seul ne donne pas, un message pouvant porter plusieurs charges. Les trois colonnes du justificatif s'écrivent ensemble ou pas du tout : une réponse différée, qui ne porte aucun document, n'en remplit aucune.
...
> [!NOTE]
> **L'adresse est consignée telle qu'elle est arrivée, sans être vérifiée.** C'est la même règle que pour le corps RegRep : un journal consigne, il ne valide pas — et une adresse que la France a refusé de suivre est précisément celle qu'un litige portera. La vérification du schéma reste là où elle décide de quelque chose : `IncomingMessage::SettleExchange`, qui y envoie un usager, et `external_link`, qui en fait un lien sur la fiche. Ni cette colonne ni le `cid:` ne sont chiffrés : le critère est de porter le sujet du justificatif, et ni l'une ni l'autre ne le porte — la même adresse vit d'ailleurs en clair sur `exchanges` et repart en clair au fournisseur de service français.
