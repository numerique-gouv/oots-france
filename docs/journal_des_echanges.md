...
Un événement par fait, dans `audit_events` (`AuditEvent`), écrit par `AuditTrail` (`app/lib/`). Ce qu'une enveloppe entrante dit d'elle-même, c'est `JournalledMessage` (`app/parsers/`) qui le lit, et `AuditTrail` qui l'enregistre : la lecture ne décide de rien et s'éprouve sans qu'aucune ligne soit écrite. Douze types :
...
>
> Le même empêchement vaut sur l'**agent requêteur**, et pour la même raison lue une couche plus haut : une `rs:Exception` le nomme en recopiant son identifiant, son schéma, son nom et la langue de ce nom, que [`R-EDM-ERR-C010`, `C027`, `C028` et `C029`](https://code.europa.eu/oots/tdd/tdd_chapters/-/blob/2.0.1/OOTS-EDM/sch/EDM-ERR-C.sch) jugent toutes en `FATAL`. Un refus fondé sur l'une de ces quatre valeurs ne peut donc pas partir non plus, et `EvidenceProvision::RejectUnanswerableRequester` l'écrit dans `request_refused` sous la règle de la requête qu'il applique. Ce que l'agent déclarait par ailleurs de lisible — son identifiant, son pays — y est consigné, quand bien même il est refusé sur autre chose : rien de cette ligne ne repart, un `AuditEvent` n'étant pas un message.
>
> **Un refus fondé sur le *nombre* d'agents requêteurs ne consigne ni identifiant ni pays**, faute d'un agent qui soit le requêteur : [`R-EDM-REQ-C074`](https://code.europa.eu/oots/tdd/tdd_chapters/-/blob/2.0.1/OOTS-EDM/sch/EDM-REQ-C.sch) veut exactement un agent classé `ER` dans la collection, [`R-EDM-REQ-S012`](https://code.europa.eu/oots/tdd/tdd_chapters/-/blob/2.0.1/OOTS-EDM/sch/EDM-REQ-S.sch) exactement un slot `EvidenceRequester` pour la porter, et [`R-EDM-REQ-S039`](https://code.europa.eu/oots/tdd/tdd_chapters/-/blob/2.0.1/OOTS-EDM/sch/EDM-REQ-S.sch) au moins un `rim:Element` qui porte un agent. Sans agent il n'y a personne à lire, avec deux aucune règle ne dit lequel est le requêteur, et consigner l'un des deux présenterait comme lu ce que la requête n'a pas dit. La ligne porte alors la règle, le code de démarche et l'échange, et ces deux colonnes restent vides — c'est la lecture du requêteur elle-même qui échoue, et tout ce que la ligne sait de lui passe par elle.

> [!NOTE]
> **Une réponse reçue consigne dans `detail` les règles du chapitre 4.6 qu'elle enfreint**, chacune avec ce qu'elle dit — la même colonne que celle où un refus nomme la règle qu'il applique. Rien n'est refusé pour autant : le [chapitre 4.6](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932928) n'attribue le devoir de valider à personne et ne relie aucune violation à un code `EDM:ERR:*` du côté du récepteur, et le [chapitre 4.5.3](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932938) n'ouvre aucun chemin d'erreur d'un portail vers un fournisseur. Écarter une réponse qu'un correspondant a légitimement émise ferait donc perdre un justificatif sans que personne l'apprenne, là où la ligne du journal se relit après coup. Les règles retenues sont les quarante-sept qu'un lecteur tranche sur le seul document, sans schéma ni liste de codes — `EvidenceResponseParser#violations` les porte, et une réponse conforme laisse le champ vide. Vingt-cinq d'entre elles jugent l'empaquetage que la 2.0 a introduit : ce que la réponse porte au premier niveau, ce que chaque objet du paquet est classé, ce qu'il identifie et référence, comment les objets se relient, et ce que le `sdg:Evidence` de chacun porte — `R-EDM-RESP-S046` à `S066` et le couple `S033` à `S037`, chacune dans la forme de la ligne que la réponse **annonce**, celle-là même qui décide déjà où se lisent les métadonnées. Les documents complémentaires qu'un paquet transporte — annexe, traduction, version lisible — sont donc lus pour être jugés et **jamais remis** : la France remet au portail la seule partie `application/pdf` que l'entête ebMS déclare, retrouvée par son type et non par le `rim:RepositoryItemRef` du `MainEvidence`.
>
> **Une réponse d'erreur reçue consigne les siennes de la même façon**, dans le `detail` de sa ligne `error_received` et à côté du message que l'exception y porte déjà : les cinquante-trois règles `R-EDM-ERR-*` que la 2.0.1 publie en `FATAL` — quatre autres y étant `CAUTION`, et la 1.2.5 en publiant quatre de plus sur le slot `PreviewMethod` que la 2.0 a retiré —, plus les cardinalités que les tables du [chapitre 4.5.3](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932938) fixent et qu'aucune assertion `FATAL` n'exécute : le slot `EvidenceRequester` présent sauf sur une `rs:InvalidRequestExceptionType`, les deux slots de prévisualisation jamais doublés, un `sdg:Agent` par slot portant son identifiant et au moins un nom. L'échange se règle exactement comme avant — en échec sur le code reçu, en attente de prévisualisation quand la sévérité et l'adresse le demandent, ou abandonné sans code quand le document n'est pas lisible. **Ce dernier cas nomme désormais sa règle** au lieu de ne rien dire : la ligne du journal s'écrit avant que l'échange soit dispatché, donc une réponse d'erreur dont la racine n'est pas une `query:QueryResponse` porte `R-EDM-ERR-S001` dans son `detail`, et une réponse sans `rs:Exception` y porte `S008`.
>
> **Le désaccord entre les deux annonces de version d'un même message est consigné sous le nom du chapitre**, `TDD 4.7 §2.6.2`, avec les deux valeurs telles qu'elles sont arrivées, et **en tête des règles** que la ligne porte — ce qui le place au début du `detail` de `response_received`, dont c'est le seul contenu, et à la suite du message de l'exception dans celui d'`error_received`. C'est un chapitre et non une règle qui est nommé, parce qu'aucune assertion publiée ne croise l'entête et le corps : les `.sch` des corps ne lient aucun préfixe `eb:`, et [`EDM-ebMS.sch`](https://code.europa.eu/oots/tdd/tdd_chapters/-/blob/2.0.1/OOTS-EDM/sch/EDM-ebMS.sch) ni `rim` ni `query`. Le constat vient en tête parce qu'il dit que le message est invalide en entier, et parce qu'il explique les violations d'empaquetage qui le suivent — un corps lu dans la ligne que son entête annonce enfreint les règles de la ligne que son propre slot revendique. Rien n'est refusé pour autant, pour la raison que le premier paragraphe donne déjà de toute violation d'une réponse reçue, et pour une de plus : le chapitre ne nomme aucun sujet pour l'erreur qu'il réclame, et un requêteur n'a personne à qui l'adresser. Trois cas n'en sont pas un, et ne consignent rien à ce titre : un entête de la ligne 1.2, qui ne porte pas la propriété ; un corps sans slot `SpecificationIdentifier`, que `R-EDM-RESP-S009` et `R-EDM-ERR-S009` comptent déjà ; et un corps illisible, qui n'exprime aucun identifiant. Ce que `R-EDM-RESP-C002` et `R-EDM-ERR-C001` disent du slot contre la version de l'**échange** reste consigné à côté : l'un des deux constats compare la réponse à la requête qu'elle règle, l'autre la réponse à elle-même. Sur une requête reçue, le même désaccord est refusé et non consigné — [versions_tdd.md](versions_tdd.md#comment-une-version-est-annoncée-dans-les-échanges) tient les deux côte à côte.
...
- **« *Preview Location* »** — étoilée pour le requêteur dans **les deux** flux, et pour le fournisseur dans le seul flux réponse. C'est `preview_location`, lue par `JournalledMessage#received_error` : l'adresse où le correspondant demande que l'usager soit envoyé avant de redemander. Le [chapitre 4.4 §4.3.2](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932919) dit ce qu'elle vaut au-delà du détour — la requête suivante la réutilise, ce qui « *may provide an additional correlation of complex flows beside `ExchangeId`* ».
...
`R-EDM-RESP-S041` et `-S042` bornent ce qui se lit, plus étroitement que la requête : l'identifiant eIDAS, le nom, le prénom, la date de naissance et le lieu de naissance côté personne physique — ni le niveau de garantie ni le sexe, que la règle exclut d'une réponse alors que la requête les porte ; l'identifiant eIDAS et la raison sociale **seuls** côté personne morale, les identifiants sectoriels du [chapitre 4.5.1](https://ec.europa.eu/digital-building-blocks/sites/spaces/TDD/pages/973932961) exclus. Ce que ces deux règles excluent n'est pas seulement laissé de côté : il est consigné, chacune étant opposée à tout objet de la liste qui porte un sujet — annexe comprise, leurs contextes ne filtrant aucune classification. Une réponse différée, qui n'annonce aucune métadonnée, ne porte donc aucun sujet. Une réponse **émise** par la France non plus : le sujet qu'elle a lu de la requête reçue figure déjà sur la ligne `request_received` du même échange, et il n'y a là aucun écart à voir.
...
> [!IMPORTANT]
> **La comparaison ci-dessus suppose qu'on tienne déjà le document contesté ; la France, elle, n'en garde jamais les octets.** Ce que le journal conserve d'un justificatif est son empreinte et la ligne qui la porte, et c'est vrai des deux sens : d'un document reçu d'un correspondant, que rien ne réécrit ; et du document que la France sert, que `EvidenceDocumentBuilder` engendre au moment de répondre et ne dépose nulle part. Un document engendré ne se reproduit donc pas à l'identique après coup — il porte l'identifiant et l'instant de la réponse qui l'a produit. La ligne `response_sent` est ce qui reste, et elle suffit à confirmer ou à démentir un document qu'une partie présente ; elle ne permet pas d'en refabriquer un.
