...
>
> **Un refus fondé sur le *nombre* d'agents requêteurs ne consigne ni identifiant ni pays**, faute d'un agent qui soit le requêteur : [`R-EDM-REQ-C074`](https://code.europa.eu/oots/tdd/tdd_chapters/-/blob/2.0.1/OOTS-EDM/sch/EDM-REQ-C.sch) veut exactement un agent classé `ER` dans la collection et [`R-EDM-REQ-S012`](https://code.europa.eu/oots/tdd/tdd_chapters/-/blob/2.0.1/OOTS-EDM/sch/EDM-REQ-S.sch) exactement un slot `EvidenceRequester` pour la porter. Sans agent il n'y a personne à lire, avec deux aucune règle ne dit lequel est le requêteur, et consigner l'un des deux présenterait comme lu ce que la requête n'a pas dit. La ligne porte alors la règle, le code de démarche et l'échange, et ces deux colonnes restent vides — c'est la lecture du requêteur elle-même qui échoue, et tout ce que la ligne sait de lui passe par elle.
