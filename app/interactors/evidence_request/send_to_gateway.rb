module EvidenceRequest
  ...
  class SendToGateway < ApplicationInteractor
    ...
    # that is the same problem — the gateway — not a fault of theirs.
      ...
      abandon(exchange, :gateway_refused, e.message)
    end

    private

    # No EDM code on either branch: the eight exceptions of chapter 4.5.3 are
    # all generated by a server handling a client's request, and none names a
    # sender whose own submission never left. The description carries the reason
    # instead — to the exchange the console reads back, and to the caller.
    def abandon(exchange, key, description)
    ...
    # correspondent's real answer as `already_settled`.
      ...
      abandon(exchange, :invalid_configuration,
