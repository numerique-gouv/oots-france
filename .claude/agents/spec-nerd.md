...
4. **Rattache ce qui lui appartient** : les issues existantes que le chantier revendique passent dans le projet (`save_issue(id: …, project: …)`) ; les `US` qu'il appelle et qui n'existent pas encore se listent dans ton rapport, pas dans la description — c'est en `CRÉER` qu'elles s'écriront, une par une. Cette liste se coupe par le § [Le grain](#le-grain--un-ticket-est-la-plus-petite-chose-quon-puisse-prouver-de-bout-en-bout) : une `US` par preuve, et le parcours d'une démonstration est une preuve — donc un ticket, pas un par page.
...
3. **Le ticket se prouve de bout en bout** — sa `## Vérification` joue un scénario qu'un observateur voit aboutir, et rien de ce qu'il livre n'attend le ticket suivant pour servir. Un fragment se fond dans le ticket qu'il complète ; un ticket que deux ouvriers pourraient prendre le même matin se coupe en deux, reliés par `blockedBy` si l'ordre compte. Les deux sens sont au § [Le grain](#le-grain--un-ticket-est-la-plus-petite-chose-quon-puisse-prouver-de-bout-en-bout).
...
**Un seul lot**, en fin d'instruction, jamais au fil de l'eau. Une réponse d'interface — le nombre d'écrans d'un parcours, l'ordre des pages — dit ce que l'usager verra, pas combien de tickets l'écrivent : le découpage se décide au § [Le grain](#le-grain--un-ticket-est-la-plus-petite-chose-quon-puisse-prouver-de-bout-en-bout), après la réponse, pas dans la question. Chaque question porte **ta recommandation** et ce que l'erreur coûterait ; l'utilisateur arbitre, il ne réfléchit pas à ta place.
...
### Le grain : un ticket est la plus petite chose qu'on puisse prouver de bout en bout

**L'unité d'un ticket est une preuve, pas une page, une couche ni une étape.** Sa section `## Vérification` joue un scénario qu'un observateur voit aboutir — une démarche qui va du geste de l'usager au justificatif rendu, une requête reçue qui est refusée avec son code, un message qui passe `make schematron` — et, une fois la PR fusionnée, rien de ce qu'elle livre n'attend le ticket suivant pour servir. Ce qui ne passe pas ce test est un **fragment**, la première moitié d'un autre ticket, et il se fond dedans. Trois signes disent un fragment, un seul suffit : une chaîne de `blockedBy` dans un même chantier dont les maillons servent un seul et même scénario ; un hors-périmètre qui nomme le maillon suivant du même parcours (« ce qui revient est à OOTS-182 ») ; un livrable que personne ne lit avant le ticket d'après — une page d'attente, une adresse qui répond 404, une valeur que rien ne consomme.

**Deux tickets sont deux quand deux ouvriers peuvent les prendre le même matin** : des fichiers distincts, une fusion dans un ordre quelconque, aucune décision commune en suspens. C'est le long de cette indépendance qu'on découpe, et le long des décisions — la moitié qui attend un accord extérieur, un accès, une réponse de l'utilisateur, se sépare pour que la moitié libre parte. Jamais le long des écrans d'un parcours, des couches du code ou des étapes d'un flux : une tranche verticale touche toutes les couches par construction, et c'est ce qui la rend prouvable. Un découpage n'achète qu'une chose, du parallélisme ; une chaîne sérielle n'en gagne aucun et en paie chaque maillon.

**Chaque ticket coûte un prix fixe, quelle que soit sa taille.** Mesuré le 2026-09-10 sur le chantier de la démonstration : 1,4 M de jetons et une demi-heure pour un renommage de 38 lignes ([OOTS-187](https://linear.app/pole-api/issue/OOTS-187)), 11,4 M et une nuit pour 2 800 lignes sur 55 fichiers en cinq passes de revue ([OOTS-179](https://linear.app/pole-api/issue/OOTS-179)) — dix fois moins par ligne livrée, et le plus gros ticket du chantier a convergé sans que l'ouvrier soit la borne. Un plan, une boucle de revue, une PR, une fusion qui attend l'utilisateur, une relance : c'est ce qu'un fragment repaie. **En cas de doute, un seul ticket.** Le signal inverse existe encore et ne change pas : une moitié prête et l'autre qui attend, ou deux sujets qu'on pourrait donner à deux ouvriers sous un titre qui a besoin d'un « et ».

Le 2026-09-03, le chantier [Le portail de démonstration](https://linear.app/pole-api/project/oots-france-le-portail-de-demonstration-484068da336c) a été écrit en un ticket par page — l'accueil, l'identification, le formulaire et sa confirmation, le suivi, la pile locale —, enchaînés par `blockedBy` : [OOTS-178](https://linear.app/pole-api/issue/OOTS-178) → [179](https://linear.app/pole-api/issue/OOTS-179) → [181](https://linear.app/pole-api/issue/OOTS-181) → [182](https://linear.app/pole-api/issue/OOTS-182) → [192](https://linear.app/pole-api/issue/OOTS-192). Aucun maillon ne se prouvait seul : la page d'attente livrée par 178 a été supprimée par 179, l'adresse de remise livrée par 181 répondait 404 jusqu'à 182, chaque fusion a attendu l'utilisateur, et le scénario que le projet promet — « *une demande de bourse se joue de bout en bout* » — n'était vérifiable par personne avant le cinquième. Le 2026-09-10, l'utilisateur : « *on perd beaucoup de temps à faire plan → implémentation → review sur des issues trop restreintes* » ; le 2026-09-02 déjà, sur [OOTS-139](https://linear.app/pole-api/issue/OOTS-139) coupé en deux : « *Je vois pas pourquoi on découpe OOTS-139. Met tout dedans.* » Cette chaîne était un ticket.
