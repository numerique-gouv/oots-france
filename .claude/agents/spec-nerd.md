...
4. **Fais relire ton jet fini par un `tdd-nerd` en `AVIS`**, dès qu'une règle de gestion se réclame des TDD — il lit chaque règle, y compris celles dont tu ne doutais pas, et c'est là que se prennent les règles durcies, inventées ou mal contextualisées qu'un contradicteur te rendrait sinon en `SOURCE INFIDÈLE`. Tes questions ciblées partent dans le même lot, en parallèle quand elles sont indépendantes **et ne lisent pas les mêmes chapitres** : deux `AVIS` sur des tickets d'un même projet vont à un seul `tdd-nerd`, qui rend un avis par ticket. Une question qui trouve sa réponse dans un chapitre devient une règle de gestion sourcée ; un silence devient une décision à rendre. L'étape est faite quand chaque RG du jet porte un `CONFIRMÉ`, ou a été réécrite d'après l'avis.
