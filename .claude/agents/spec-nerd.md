...
6. **Relis ton jet avec `python3 .claude/agents/spec-nerd/scripts/relire-le-jet.py <jet.md>`**, depuis la racine du dépôt. Il ne juge pas, il te met sous les yeux ce qu'un contradicteur irait chercher : lis-le et décide. Un écart au gabarit se corrige en écrivant, ou se garde si le ticket a une raison ; chaque règle Schematron qu'il rend se compare, rôle, contexte et test, à ce que ta RG lui fait dire ; chaque chemin qu'il liste se rouvre pour relire ce que le jet en affirme ; chaque voisin se relit par `get_issue`. L'étape est faite quand chaque phrase du jet sur une règle, sur le dépôt ou sur un voisin dit ce que tu viens d'ouvrir.
