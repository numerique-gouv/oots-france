...
4. Rejoue la vérification **en entier** — `make test`, `make schematron`, et le bout-en-bout dans la CI si l'un des deux côtés touche aux charges ebMS. Ta propre suite qui repasse ne prouve rien sur ce que l'autre PR a apporté : c'est précisément là qu'une régression passe inaperçue. Vérifie nommément que ce que l'autre a ajouté est toujours là.
5. Repousse en `--force-with-lease` — jamais `--force` nu —, puis **redéclare `review`** : le rebase fini, tu retrouves l'état du § 5 — PR ouverte et convergée — et il ne te reste que la CI à attendre, par `ci-en-fond` ; ne rejoue pas `ship-plan`.
