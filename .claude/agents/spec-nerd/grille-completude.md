...
3. **La source dit ce que le ticket lui fait dire.** C'est le contrôle qui coûte, et il porte sur les RG qui se réclament des TDD : sur ton jet, l'`AVIS` de l'étape 4 a lu chaque RG, et tu rouvres toi-même, dans `.schematron/2.0.1/sch/` quand elle y est, toute règle que tu as réécrite depuis — une assertion se lit en une requête là où une page de prose se discute ; sur le texte enregistré, un nouvel `AVIS` seulement si une RG a changé depuis le sien. Une RG fondée sur une décision locale se relit contre le commentaire qui l'a rendue, pas contre un chapitre. Trois écarts, par fréquence : une règle durcie, une règle inventée, un vocabulaire local — [`docs/glossaire.md`](../../../docs/glossaire.md) tranche le dernier.
