...
Spécifié contre `main` à `<commit>`, le <date>.
...
- **Le contexte ne porte aucun fait qu'une RG ou un CA n'utilise.** Chaque affirmation vérifiable — un compte de règles, ce qu'un fichier fait, ce qu'un voisin porte — est une affirmation qu'un relecteur ira vérifier, et celle qu'aucune règle n'exploite lui coûte cette vérification pour rien : le 2026-09-17, deux jets de 766 et 1 096 mots de contexte ont reçu un quart de leurs incohérences sur ce seul paragraphe. L'enquête qui a mené au ticket va en commentaire si elle doit survivre, pas dans la description. Le contexte finit par la base — le commit de `main` et la date — pour que `relire-le-jet.py` dise ce qui a bougé depuis, et qu'un lecteur sache contre quoi le ticket a été vérifié.
