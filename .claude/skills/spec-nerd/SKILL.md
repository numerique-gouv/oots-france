...
description: >
  ...
  ticket —, ou ouvre le projet Linear d'un chantier. Confronte aux TDD, par
  des sous-agents tdd-nerd, toute question du domaine avant de la poser —
  pas l'outillage, la console ni la CI —, puis pose en un seul lot, par
...
- **Les sous-agents `tdd-nerd` se lancent quand le sujet touche au domaine, et comme l'agent le dit** : `Agent(subagent_type: "tdd-nerd", description: "TDD-nerd <sujet>", prompt: …)`, plusieurs en parallèle quand les questions sont indépendantes. Nomme le service attendu et ce que tu veux voir cité.
