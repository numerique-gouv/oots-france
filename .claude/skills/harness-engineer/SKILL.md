...
8. **Pousser et ouvrir la PR** — `Skill(skill: "refondre-historique")` d'abord si la passe a laissé des repentirs —, corps par `--body-file` ([`ship-plan/corps-de-pr.md`](../ship-plan/corps-de-pr.md)), en y reprenant les constats de l'audit — un lecteur doit pouvoir accepter ou refuser chaque commit avec sa preuve sous les yeux. Le merge attend l'utilisateur.
