...
4. **Écrire l'audit** dans `.claude/audits/AAAA-MM-JJ-harnais.md`, **avant de toucher à un fichier** — un diagnostic écrit après les correctifs les justifie au lieu de les précéder. Format au § « Ce que tu rends ».
5. **Appliquer**, sur une branche `harnais-<sujet>` depuis un `main` à jour, un commit par constat, message en français à l'impératif, sans trailer. Ce qui s'applique sans demander : les corrections d'entropie, les rapatriements, les déplacements et les raccourcissements. Ce qui se propose dans l'audit et attend une réponse : retirer une règle, ajouter un hook ou une permission à `settings.json`, changer un verdict ou un format que la statusline lit.
