...
8. **Pousser (`git push -u origin $(git branch --show-current)`) et ouvrir la PR**, corps par `--body-file` ([`ship-plan/corps-de-pr.md`](../ship-plan/corps-de-pr.md)), en y reprenant les constats de l'audit — un lecteur doit pouvoir accepter ou refuser chaque commit avec sa preuve sous les yeux. Le merge attend l'utilisateur. La PR ouverte, si la passe a laissé des repentirs, `Skill(skill: "refondre-historique")`.
9. **Mesurer et dater.** L'audit porte les chiffres de la fenêtre, avec les commandes de [`mesures.md`](mesures.md) et `Skill(skill: "mesurer-les-jetons")` pour les jetons, pour que la passe suivante compare. Un chiffre ne s'écrit dans un skill qu'avec sa date et sa mesure ; les tiens vont dans l'audit.
