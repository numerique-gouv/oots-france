...
- **La description est un pointeur, pas un résumé** : ce que le skill fait en une phrase, puis les déclencheurs, un par branche, à la troisième personne. Le contrat est dans le corps, chargé à l'invocation seulement. Un skill qu'aucun rôle n'appelle porte `disable-model-invocation: true` : vérifié le 2026-09-15 sur Claude Code 2.1.270, il disparaît alors de la liste des skills de la session — sa description ne coûte plus rien à chaque tour, l'outil `Skill` le refuse, et seul `/<nom>` tapé par l'utilisateur l'invoque.
...
- **L'heure de cache du frontmatter d'un agent** (`experimental: cacheTtl: 1h`) a été essayée le 2026-09-09 et retirée : 5 % de gain dans son meilleur cas mesuré, et une attente sauvée de moins la fait basculer en surcoût (`orchestrateur/couts.md`). Elle ne redevient défendable que sur un rôle dont plus de 40 % du cache écrit est recréé après une attente, mesuré sur plusieurs passes — les deux termes, jamais le seul nombre de reprises à froid.
