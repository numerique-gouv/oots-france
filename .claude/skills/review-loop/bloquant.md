...
- **Non-bloquant**, tout le reste — explicitement, y compris :
  ...
  - **duplication / redite**, y compris une violation de « Documentation: one fact, one place » de `CLAUDE.md` — sauf si les deux copies ont déjà divergé en substance dans du **code** (l'une fait autre chose que l'autre : bug de correctness, donc bloquant par le premier critère, pas par la duplication elle-même) ;
