...
Le portier est propre au poste, et absent d'un autre : ne l'appelle que s'il existe.

```sh
Q=~/.claude/hooks/peon-quiet.sh
[ -x "$Q" ] && "$Q" on     # avant la première passe
[ -x "$Q" ] && "$Q" off    # avant de rendre la main, quelle qu'en soit la raison
```

Le `off` n'est pas optionnel et ne vaut pas que pour la fin normale (étape 6, branche « Non ») : **tout** endroit où la boucle rend la main à l'utilisateur le réclame d'abord — finding ambigu (étape 4), CI rouge deux fois ou échec d'infra (étape 4bis), arbitrage d'oscillation (`non-convergence.md`, point 3), règle des 5 passes. Sinon la question part sans son, ce qui est exactement le contraire du but : on coupe le bruit pour que le silence redevienne informatif.
