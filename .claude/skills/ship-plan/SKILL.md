...
3. **Ouvrir la PR, l'attacher au ticket, et passer `In Review`.** Titre et corps selon [`corps-de-pr.md`](corps-de-pr.md) — dérivés du plan, par `--body-file`, jamais `--body` en ligne. Si une PR existe déjà pour cette branche (`gh pr view` réussit), reprendre son URL, et la sortir du brouillon si l'ouvrier l'y avait laissée. Puis, en une seule écriture, `save_issue(id: "OOTS-nn", state: "In Review", links: [{url: "<url-de-la-PR>", title: "PR #<n> — <titre>"}])` — `links` est cumulatif, vérifier avec `get_issue` que la PR n'y figure pas déjà. Commenter seulement si l'implémentation a divergé du plan — un lot repoussé, une solution remplacée, un point devenu sans objet — avec ce qui a changé et pourquoi, jamais un résumé du diff.
