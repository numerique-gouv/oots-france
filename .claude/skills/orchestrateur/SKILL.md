...
Chaque adresse va avec **ce qu'on y regarde**, en une ligne, et chaque ligne porte **l'URL entière** — `http://localhost:3001/admin/journal/events/19` —, jamais un chemin nu sous une base écrite une fois en tête de tableau : l'utilisateur clique dans son terminal, et un chemin ne se clique pas. Dit le 2026-09-11 sur les écrans d'OOTS-206 (« donne le lien complet dude »). Un port et une route ne disent pas non plus pourquoi on les ouvre. Vérifie-les joignables avant de les donner (`curl -so /dev/null -w '%{http_code}' <url>` ; un `303` est normal, la console est protégée), et **dis ce qui ne s'y voit pas et pourquoi** — une page qui ne montre pas le cas traité, faute de données réelles qui le produisent, est une déception à annoncer plutôt qu'à laisser découvrir.
