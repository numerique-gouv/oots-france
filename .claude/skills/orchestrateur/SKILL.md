...
- **Ne relance pas un second ouvrier sur le même ticket** tant que le premier tient un travail en cours : reprends-le par `SendMessage`. Deux exceptions, où le contexte vide est ce qu'on veut : après `PLANIFIÉ` ou `PLAN` résolu, et un ouvrier arrêté tard dont ce qui reste tient sans son historique. `ÉCRAN` n'en fait pas partie.
